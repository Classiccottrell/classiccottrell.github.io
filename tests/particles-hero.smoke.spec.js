...
// Smoke coverage for the ripple-ring feature: sweeping the pointer across the
// field should spawn expanding ripple rings that disturb nearby nodes/particles
// as they travel outward, and settle back out without throwing.
test('moving the pointer across the hero field produces a ripple without errors', async ({ page }) => {
  ...
  // Sweep the pointer through the field, exercising the ripple-spawn +
  // applyRipple() per-node and per-sky-particle impulse path.
